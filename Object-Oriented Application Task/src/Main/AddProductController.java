...
/**This is the controller for the AddProduct.fxml file*/
public class AddProductController implements Initializable {
    ...
    /**Add Part Button, used to associate parts with a product
     * RUNTIME ERROR: Fixed, trying to add part without a part selected resulted in
    ...
    /**Delete Part Button, used to remove parts from a product*/
    @FXML
    ...
    void saveProduct(ActionEvent event) {
        ...
        for (int i = 0; i < partsToAdd.size(); i++) {
            partsPrice += partsToAdd.get(i).getPrice();
        }
    ...
    /**Used to search through the parts list
     * LOGICAL ERROR: Fixed, search by ID previously wasn't implemented, you can now
     * search by ID.
     * */
    @FXML
    void searchPart(ActionEvent event) {
        ...
        if (!searchText.isEmpty()) {
            try{
                ...
                PartSearchTable_A.setItems(Inventory.lookupPartID(searchId));
